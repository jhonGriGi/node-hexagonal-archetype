...
import type { CreateProductCommandHandler } from '@domain/command/create_product/command_handler';
...
export class CreateProductHandler implements LambdaHandlerInterface {
    ...
    public async handler(
        ...
        try {
            ...
            const createProductParsed: CreateProductDTO = ApiGatewayEnvelope.parse(
                event,
                CreateProductSchema,
            );
        ...
        = await this.commandHandler.execute(createProductParsed);
            return ApiResponseBuilder.empty()
                .withStatusCode(200)
                .withHeaders({ 'Content-Type': 'application/json' })
